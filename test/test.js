...
describe("dom[selector]", function() {
  ...
  it("should be a function", function() {
    expect(typeof dom).to.equal("function");
  });

  it("can accept no argument", function() {
    expect(typeof dom()).to.equal('object', typeof dom());
  });

  it("can select by class", function() {
    expect(dom.class.demo.length).to.equal(1);
  });

  it("can select by tag", function() {
    expect(dom.body.length).to.equal(1);
  });

  it("can select by id", function() {
    expect(dom.id.demo.length).to.equal(1);
  });

  it("can select with css", function() {
    expect(dom['[id="demo"]'].length).to.equal(1);
    expect(dom['.demo ul'].length).to.equal(1);
  });
});



describe("dom[selector].class", function() {
  ...
  it("#class-arrays: should be an array", function() {
    expect(typeof dom.id.base.class).to.equal('object');
    expect(dom.id.base.class instanceof Array).to.equal(true);
  });

  it("#class-arrays: retrieves a deep list of classes", function(){
    expect(dom.id.base.class[0]).to.deep.equal(['base']);
    expect(dom.id.base.class[0][0]).to.deep.equal('base');
    // For some reason this one doesn't work:
    // expect(dom.id.base.class).to.deep.equal([['base']]);
    expect(str(dom.id.base.class)).to.deep.equal(str([['base']]));
  });

  it("#class-flat: retrieves a simple list of classes", function(){
    base.addClass('bla');
    expect(dom.id.base.class._flat).to.deep.equal(['base', 'bla']);
  });

  it("#class-text: retrieves a text list of classes", function(){
    base.addClass('bla');
    expect(dom.id.base.class._text).to.deep.equal('base bla');
  });

  it("#class-one: can check for a particular class", function() {
    expect(dom.id.base.class.base).to.equal(true);
    expect(dom.id.base.class.nonexisting).to.equal(false);
  });

  it("#class-push: can add a single class with equal", function() {
    expect(base.hasClass('bla')).to.equal(false);
    dom.id.base.class = 'bla';
    expect(base.hasClass('bla')).to.equal(true);
  });

  it("#class-make-true: can add a single class with true", function() {
    expect(base.hasClass('bla')).to.equal(false);
    dom.id.base.class.bla = true;
    expect(base.hasClass('bla')).to.equal(true);
  });

  // Not available so far
  // it("can remove a single class with delete", function() {
  //   base.addClass('bla');
  //   expect(base.hasClass('bla')).to.equal(true);
  //   delete dom.id.base.class.bla;
  //   expect(base.hasClass('bla')).to.equal(false);
  // });

  it("#class-make-false: can remove a single class with false ", function() {
    base.addClass('bla');
    expect(base.hasClass('bla')).to.equal(true);
    dom.id.base.class.bla = false;
    expect(base.hasClass('bla')).to.equal(false);
  });
});
...
describe("dom[selector][attribute]", function() {
  ...
  it("#attr-alias get the html of .insides", function(){
    expect(str(dom.class.insides.html)).to.deep.equal(str(['a <span>thing</span>']));
    expect(dom.class.insides.html[0]).to.deep.equal('a <span>thing</span>');
  });

  it("#attr-alias change the html of #world", function(){
    dom.id.world.html = 'Hello there';
    expect($('#world').html()).to.equal('Hello there');
  });
  ...
  it("can add a single class with true", function() {
    dom.id.base.class.bla = true;
    expect(base.hasClass('bla')).to.equal(true);
  });
  ...
  it("can remove a single class with false", function() {
    base.addClass('bla');
    expect(base.hasClass('bla')).to.equal(true);
    dom.id.base.class.bla = false;
    expect(base.hasClass('bla')).to.equal(false);
